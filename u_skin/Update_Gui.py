...
class Local:
    ...
    def _run_leagueskin():
        ...
        print('正在运行最新版本的LeagueSkin.')
        try:
            win32api.ShellExecute(0, 'open', 'C:\\Fraps\\%s' % file_name, '', '', 1)
        except:
            print('权限不足.')
...
class Network:
    ...
    def get_network_version(self):
        rec = re.compile(r'DOWNLOAD\sLOL\sPRO\s\d+\.\d+\.\d+\shere')
        try:
    ...
    def get_download_url(self):
        rec = re.compile(r'http://dl2\.modskinpro\.com/LEAGUESKIN_\d+\.\d+\.\d+\.zip\?update=\d+')
        try:
    ...
    def download_update(self):
        ...
        try:
            ...
            with open('new_version.zip', "wb") as f:
                for chunk in download_tab.iter_content(chunk_size=32):
                    if chunk:
                        temp_size += len(chunk)
                        f.write(chunk)
                        f.flush()
